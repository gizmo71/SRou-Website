...
$query = lm2_query("SELECT DISTINCT id_event_group, short_desc, t.id_poll, reg_topic
...
function do_event_group_poll($event_group, $desc, $poll, $topic) {
	...
	$query = lm2_query("
		...
		WHERE champ_group_type <> 'L' AND id_event_group = $event_group
		", __FILE__, __LINE__);
	while ($row = mysql_fetch_assoc($query)) {
	...
	if (count($membergroups) > 1) {
		...
		$query = lm2_query("
			...
			WHERE (CONCAT(',', additionalGroups, ',') REGEXP CONCAT(',',g.id_group,',')
		...
		$query = lm2_query("SELECT m.id_member AS member, l.id_choice AS choice
			, memberName, posts, realName, label
			, groupName, g.id_group AS id_group
			...
			LEFT JOIN {$db_prefix}log_polls AS l ON l.id_poll = $poll
			...
			LEFT JOIN {$db_prefix}membergroups g
				ON (CONCAT(',', additionalGroups, ',') REGEXP CONCAT(',',g.id_group,',')
				AND g.id_group IN (" . implode(",", $membergroups) . "))
			WHERE c.id_choice IS NOT NULL OR g.id_group IS NOT NULL
			ORDER BY id_group, c.id_choice, events DESC, realName
			", __FILE__, __LINE__);
		$current_group = null;
		while ($row = mysql_fetch_assoc($query)) {
			$group = $row['id_group'];
...
function list_all_messages($member, $topic) {
	...
	$query = lm2_query("SELECT body"
		. " FROM {$db_prefix}messages"
		. " WHERE id_topic = $topic"
 		. " AND id_member = $member"
		. " ORDER BY posterTime"
		, __FILE__, __LINE__);
	$sep = "";
	$list = "";
...
function list_all_choices($member, $poll) {
	...
	$query = lm2_query("SELECT c.id_choice AS id, label"
		. " FROM {$db_prefix}log_polls AS l"
		. ", {$db_prefix}poll_choices AS c"
		. " WHERE l.id_member = $member"
 		. " AND l.id_poll = $poll"
 		. " AND c.id_poll = $poll"
 		. " AND l.id_choice = c.id_choice"
...
function list_members_groups($member, $membergroups) {
	...
	$query = lm2_query("
		...
		JOIN {$db_prefix}members AS m ON CONCAT(',', m.id_group, ',', additionalGroups, ',') REGEXP CONCAT(',',g.id_group,',')
...
function list_members_teams($member) {
	...
	$query = lm2_query("SELECT DISTINCT team_name
